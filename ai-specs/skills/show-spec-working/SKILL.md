...
- Optional spec context from user:
  - Direct ticket id in text (for example: `SCRUM-01`)
  - Feature name
  - Endpoint
  - Frontend route
...
1. Start required local services if needed (backend en puerto 3010, frontend en 3000).
...
3. If any call changes data state (CREATE/UPDATE/DELETE):
   - Execute the paired restore/reset curl command immediately after demonstrating the behavior.
